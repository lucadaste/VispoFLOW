...
import { Send, Check, Circle, ArrowLeftRight, Info, CalendarClock, History as HistoryIcon, FileCheck2, Trash2, MessageSquarePlus } from "lucide-react"
...
export function TransactionsOnboarding({
  ...
  /** Soft-deletes a completed document (restorable from the Document Library) without
  ...
  const [deleteConfirm, setDeleteConfirm] = useState<{ item: TransactionItem; groupTitle: string; doc: LibraryDoc } | null>(null)
  ...
  // deleting it — progress, docs, and history are untouched. Disabled while a document is open
  // (see the header button below) so the field it's waiting on never scrolls out of view.
  const startNewChat = useCallback(() => {
    setChatBreakId(idRef.current)
    setShowEarlier(false)
    pushBot("Started a new chat — pick a document from Transaction Documents to continue, or ask me anything.")
  }, [pushBot])
        ...
        <div className="border-t border-border bg-card/80 backdrop-blur px-4 py-4 sm:px-8 lg:px-12">
          <div className="mx-auto flex max-w-2xl items-end gap-2">
            <button
              ...
              className="inline-flex shrink-0 items-center justify-center gap-1.5 self-stretch rounded-xl border border-border bg-card px-2.5 text-xs font-medium text-muted-foreground shadow-sm transition-colors hover:border-primary hover:text-primary disabled:cursor-not-allowed disabled:opacity-40 sm:px-3"
            >
