import type { FlowAnswers, OptionGrant } from "@/lib/flow"
...
FORM OF NOTICE OF STOCK OPTION GRANT, STOCK OPTION AGREEMENT AND EXERCISE AGREEMENT

(See the Notice of Stock Option Grant document.)`
}
...
function blankNoticeOfOptionGrant(a: FlowAnswers): string {
  const year = currentYear()
  return `
${year} STOCK PLAN
...
function buildNoticeOfOptionGrant(a: FlowAnswers, grant: OptionGrant): string {
  const year = currentYear()
  const sharesStr = grant.shares.toLocaleString()
  const isISO = grant.optionType === "ISO"
  const priceNum = parseFloat(grant.exercisePrice.replace(/[^0-9.]/g, "")) || 0
  const totalExercisePrice = (priceNum * grant.shares).toLocaleString(undefined, {
    minimumFractionDigits: 2,
    maximumFractionDigits: 2,
  })
  const addressLines = grant.address.split("\n").map((l) => l.trim()).filter(Boolean)
  const addressLine1 = addressLines[0] ?? ""
  const addressLine2 = addressLines.slice(1).join(", ")
  const grantDate = formatOptionDate(grant.grantDate)
  const expirationDate = formatOptionDate(grant.expirationDate)
  const vestingCommencementDate = formatOptionDate(grant.vestingCommencementDate)
  const vestingDay = grant.vestingCommencementDate
    ? String(new Date(grant.vestingCommencementDate + "T00:00:00").getDate())
    : "__________"
...
Exercise Price Per Share: USD$${grant.exercisePrice}

Total Number of Shares: ${sharesStr}

Total Exercise Price: USD$${totalExercisePrice}

Type of Option: ${isISO ? sharesStr : "0"} Shares Incentive Stock Option
${isISO ? "0" : sharesStr} Shares Nonstatutory Stock Option

Expiration Date: ${expirationDate}

Vesting Commencement Date: ${vestingCommencementDate}

Vesting/Exercise Schedule: So long as your Continuous Service Status does not terminate (and provided that no vesting shall occur following the Termination Date (as defined in Section 5 of the Stock Option Agreement) unless otherwise determined by the Company in its sole discretion), the Shares underlying this Option shall vest and become exercisable in accordance with the following schedule: 1/4th of the Total Number of Shares shall vest and become exercisable on the 12-month anniversary of the Vesting Commencement Date and 1/48th of the Total Number of Shares shall vest and become exercisable on the ${vestingDay} day of each month thereafter [(and if there is no corresponding day, the last day of the month)].
...
function noticeOfOptionGrant(a: FlowAnswers): string {
  if (a.optionGrants.length === 0) {
    return blankNoticeOfOptionGrant(a)
  }

  return a.optionGrants
    .map((g) => buildNoticeOfOptionGrant(a, g))
    .join("\n\n\n════════════════════════════════════════\n\n\n")
}
...
function buildIndemnificationAgreement(a: FlowAnswers, indemniteeName: string): string {
  return `INDEMNIFICATION AGREEMENT

This Indemnification Agreement (this "Agreement") is made as of ${today()}, by and between ${a.companyName}, a Delaware corporation (the "Company"), and ${indemniteeName} ("Indemnitee").
...
The parties have executed this Indemnification Agreement as of ${today()}.
...
function indemnificationAgreement(a: FlowAnswers): string {
  const founders = a.allocations.filter((alloc) => !alloc.isPool)

  if (founders.length === 0) {
    return buildIndemnificationAgreement(a, "[No founder allocations specified]")
  }

  return founders
    .map((f) => buildIndemnificationAgreement(a, f.name))
    .join("\n\n\n════════════════════════════════════════\n\n\n")
}
...
const RENDERERS: Partial<Record<string, (a: FlowAnswers) => string>> = {
  ...
  "indemnification-agreement": indemnificationAgreement,
  bylaws,
  "option-pool": optionPool,
  "notice-of-option-grant": noticeOfOptionGrant,
}
