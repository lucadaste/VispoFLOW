...
export type ComplianceField = {
  ...
  sensitive?: boolean
  /** For a `sensitive` field whose value may belong to someone other than the account holder
   *  (e.g. a co-founder's SSN, not the person filling out the flow): `nameField` names the field
   *  — always the one immediately before this one — holding that person's name. When its value
   *  doesn't match the account holder's own name, the question for *this* field is skipped
   *  entirely and routed to an emailed invite instead, so the account holder never types someone
   *  else's SSN. See components/compliance-view.tsx's delegation handling and app/provide-info/[token]. */
  delegatable?: { nameField: string }
}
...
const EIN: ComplianceItem = {
  ...
  fields: [
    ...
    {
      name: "principalActivityCategory",
      label: "Principal business activity (Line 16)",
      question: "Which category best describes the corporation's principal business activity? Pick \"Other\" only if none of these fit.",
      type: "select",
      options: [
        "Construction",
        "Real estate",
    ...
    { name: "principalActivity", label: "Specific line of business (Line 17)", question: "What are the specific products the company sells, services it provides, or work it does?", placeholder: "e.g. Software development" },
...
const EIGHTY_THREE_B: ComplianceItem = {
  ...
  fields: [
    ...
    { name: "pricePerShare", label: "Price paid per share, in USD (Box 7a)", type: "number", question: "What price per share was paid?", placeholder: "e.g. 0.0001" },
    {
      name: "fairMarketValuePerShare",
      label: "Fair market value per share at grant, in USD (Box 6a)",
      type: "number",
      question: "What's the fair market value per share at grant?",
      placeholder: "Same as price paid, unless the stock is worth more (e.g. a later grant)",
      tooltip: "For a founder buying stock at incorporation, this is normally the same as the price paid — the company has no real value yet, so there's no gap to tax. Enter a different, higher number only if the company already has real value at the time of this grant (e.g. a non-founder grant after a priced round), in which case the difference between this and the price paid counts as taxable income on Box 8.",
    },
